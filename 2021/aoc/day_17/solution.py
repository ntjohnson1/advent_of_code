...
def find_final_positions_y_positive(max_y_velocity, max_time):
    """
    ...
    # find which speeds thrown straight down land in the region
    # anything faster than min_t when negative will be past the region
    candidate_y_velocities = np.arange(max_y_velocity+1)
    candidate_times = np.arange(1, max_time)
    # Here we treat our velocities as candidate times as well (introduces need for negative)
    final_positions = np.outer(candidate_y_velocities, candidate_times) - 0.5 * (
...
def problem_2(inputs):
    ...
    # Need to do non-negative initial velocities separately
    # because I didn't want to do a full re-write from part 1
    max_y_vel = np.max(np.where(valid_y)[0])
    max_t = 2*max_y_vel + 1 + len(fy)
    fy_pos = find_final_positions_y_positive(max_y_vel, max_t)
    valid_y_pos = (fy_pos <= max_y) & (fy_pos >= min_y)

    # Need to track pairs since some velocities yield
    # valid positions at multiple time stamps
    valid_pairs = []
    ...
    for i in range(min(len(fx), fy_pos.shape[1])):
        if np.any(valid_x[:, i]) and np.any(valid_y_pos[:, i]):
            x_velocities = np.where(valid_x[:, i])[0]+1
            y_velocities = (np.where(valid_y_pos[:, i])[0])
            valid_pairs.extend(list(product(x_velocities, y_velocities)))
